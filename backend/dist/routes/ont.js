...
router.post('/:id/wifi', async (req, res) => {
    ...
    const params = {};
    if (ssid24)
        params['InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.SSID'] = ssid24;
    if (key24)
        params['InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.PreSharedKey.1.PreSharedKey'] = key24;
    if (ssid5)
        params['InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.SSID'] = ssid5;
    if (key5)
        params['InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.PreSharedKey.1.PreSharedKey'] = key5;
    try {
